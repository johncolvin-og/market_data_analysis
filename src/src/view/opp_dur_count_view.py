...
from src.view.data_view import get_style_dict
from src.core.sort_utils import binary_search


class OppDurCountView:
    @staticmethod
    def fmt_dur_count_selector(dur_count, even_odd=None):
        parts = ['lsn-win', f'{dur_count}']
        if even_odd is not None:
            parts.append(even_odd)
        return '-'.join(parts)

    @staticmethod
    def get_dur_count_selector_column(df, col_name, odb_keys):
        ...
        return res.apply(
            lambda r:
            f'{OppDurCountView.fmt_dur_count_selector(odb_keys[r[0]])}-{r[1]}',
            axis=1)
    ...
    def __init__(self, opp, **kwargs):
        ...
        self.dur_type = kwarg_picker.pick(kwargs, 'dur_type')
    ...
    def display(self, **kwargs):
        ...
        dur_type = kwarg_picker.pick_or(
            kwargs, self.dur_type, 'dur_type') or pd.Timedelta
        ...
        def get_selector_columns(s):
            def get_dur_formatter(col):
                return lambda x: OppDurCountView.get_dur_count_selector_column(
            ...
            res = res.apply(lambda r: f'cell-{r["default_css"]}', axis=1)
            return res

        selector_cols = opp_dur_view.copy().apply(get_selector_columns)
        ...
        append_bucket_styles(opp_durs, OppDurCountView.fmt_dur_count_selector)
...
def display_opp_dur_view(odv, **kwargs):
    ...
    dur_type = kwargs.get('dur_type')
    res = odv.display(verbose=None, dur_cols=dur_cols, dur_type=dur_type)
    return res
    # return res.format({c: lambda x: TimedeltaView(x, fmt='micros') for c in dur_cols}, subset=dur_cols)
